  ...
  useEffect(() => {
    ...
    if (confirmToken) {
      ...
      api.post('settings/integrations/approve', { token: confirmToken })
        .then((res) => {
          if (res.success) {
            setSuccess((t('settingsAdvanced.664014') || '第三方整合金鑰已成功確認並套用！'));
          } else {
            setError(typeof res.error === 'string' ? res.error : (t('settingsAdvanced.9476c8') || '金鑰確認失敗，可能已過期'));
          }
          // Remove token from URL
          window.history.replaceState({}, '', window.location.pathname);
        })
        .catch(() => setError((t('settingsAdvanced.11012c') || '網路連線錯誤，無法確認金鑰')))
