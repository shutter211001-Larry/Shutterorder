...
這份指南是專為「完全沒有技術背景」的使用者設計的。我們會一步步教你如何取得 KitchenAsty 運作所需的各種「數位通行證」（金鑰）。
...
*   **為什麼需要它？** 因為當 KitchenAsty 想要代表你寄信、使用 AI 或傳送 LINE 訊息時，Google 或 LINE 需要確認「是誰在要求這項服務」以及「是否有權限」。
...
這串金鑰是讓你的 KitchenAsty 擁有 AI 靈魂的關鍵，用來處理自動化的文字分析。
...
3.  專案名稱寫 `KitchenAsty-Login`，點擊 **CREATE**。
4.  等它轉完，在專案選擇器改選剛剛建立的 `KitchenAsty-Login`。

#### 設定「同意畫面」（告訴客人你是誰）
1.  在左側搜尋框搜尋 "OAuth consent screen" 並進入。
2.  選擇 **External** (外部)，點擊 **CREATE**。
3.  填寫：
    *   **App name:** KitchenAsty
...
這部分的流程與上面「建立專案」完全一樣，但專案名稱請叫 `KitchenAsty-Email`。
...
2.  **建立 Provider：** 點擊 **Create** 建立一個供應商，名稱可以寫 `KitchenAsty-Service`。
3.  **建立 Channel：**
