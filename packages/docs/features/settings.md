...
| `googleMapsApiKey` | Google Maps API key for delivery zones |
...
| `deliveryLeadTime` | Default delivery lead time in minutes |
| `pickupLeadTime` | Default pickup lead time in minutes |
...
| `timeInterval` | Time slot interval in minutes |
...
| `stripeWebhookSecret` | Stripe webhook signing secret (masked) |
...
| `enableRateLimiting` | Enable API rate limiting |
